...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    // The app keeps its own seat and sends it with every call; a browser is
    // given one to keep. Either way the leader takes the first seat here, by
    // the name they have just given, so the board does not turn round and ask
    // them who they are.
    ...
    // The short code as well, because that is what a link says and what
    // somebody reads out to a friend standing next to them.
    ...
    return NextResponse.json({
      id: data.id,
      short: (made?.short as string | null) ?? null,
      seat: token,
      when: batch.delivery_window_text,
    });
  } catch {
    return NextResponse.json({ error: "Could not start that group." }, { status: 500 });
  }
